...
$api->version('v1', function ($api) {
	...
	$api->group(['namespace' => 'Api\Controllers', 'middleware' => '\Barryvdh\Cors\HandleCors::class'], function ($api) {
		...
		$api->group( [ 'middleware' => 'jwt.auth' ], function ($api) {
			...
			$realPath = str_replace('api',"", Request::path());
			
			if ( str_replace("/","", $realPath) == "login" || (str_replace("/","", $realPath) == "register") ) {
				// Do nun
			} else {
				$api->any($realPath, 'FakerController@resolver');
			}
			...
			// // Fakes
			// $api->get('me', 'FakerController@getMe');
			// $api->get('me/schools', 'FakerController@schools');
			// $api->get('me/notices', 'FakerController@notices');
